...
void MFRC522::PCD_WriteRegister(	byte reg,		///< The register to write to. One of the PCD_Register enums.
	...
	m_io.chipSelect->reset();
	m_io.spi->Transmit(reg & 0x7E);
	m_io.spi->Transmit(value);
	m_io.chipSelect->set();
	/*
...
void MFRC522::PCD_WriteRegister(	byte reg,		///< The register to write to. One of the PCD_Register enums.
	...
	for (byte index = 0; index < count; index++) {
		m_io.spi->Transmit(values[index]);
	}
	m_io.chipSelect->set();
	/*
	SPI.beginTransaction(SPISettings(SPI_CLOCK_DIV4, MSBFIRST, SPI_MODE0));	// Set the settings to work with SPI bus
...
byte MFRC522::PCD_ReadRegister(	byte reg	///< The register to read from. One of the PCD_Register enums.
	...
	byte value;
	m_io.chipSelect->reset();
	m_io.spi->Transmit(0x80 | (reg & 0x7E));
	value = m_io.spi->TransmitReceive(0);
	m_io.chipSelect->set();
	return value;
	/*
	byte value;
...
void MFRC522::PCD_ReadRegister(	byte reg,		///< The register to read from. One of the PCD_Register enums.
	...
	m_io.chipSelect->reset();		// Select slave
	count--;								// One read is performed outside of the loop
	m_io.spi->Transmit(address);					// Tell MFRC522 which address we want to read
	while (index < count) {
		if (index == 0 && rxAlign) {		// Only update bit positions rxAlign..7 in values[0]
			...
			byte value = m_io.spi->TransmitReceive(address);
			// Apply mask to both current value of values[0] and the new data in value.
			values[0] = (values[index] & ~mask) | (value & mask);
		}
		else { // Normal case
			values[index] = m_io.spi->TransmitReceive(address);	// Read value and tell that we want to read the same address again.
		}
		index++;
	}
	values[index] = m_io.spi->TransmitReceive(0);			// Read the final byte. Send 0 to stop reading.
	m_io.chipSelect->set();			// Release slave again
...
bool MFRC522::PCD_Init(const RC522IO& io) {
	...
	if (m_io.resetPowerDown->state() == false) {	//The MFRC522 chip is in power down mode.
		m_io.resetPowerDown->set(); // Exit power down mode. This triggers a hard reset.
